...
ALTER TABLE JYUSER ADD(api VARCHAR2(50));

UPDATE JYUSER SET upwd='3fc91467951ad439b300f4117f05c9620b7b3c6e563cfb3737bcf6d68b35d7e5b3ef6148f392b8edf76cd506f8775a690f79327c254701cb27346316a33976e2'
WHERE upwd='chat';

ALTER TABLE JYUSER MODIFY(upwd VARCHAR2(150));
...
SELECT COUNT(catid) AS catcount, b.CATID FROM JYBOARD b 
WHERE CATID != 0 AND NOTICE = 0 
GROUP BY CATID ORDER BY catcount DESC;
...
SELECT * FROM JYBOARD WHERE SYSDATE - WDATE <= 2;
...
CREATE TABLE JYBOOK(
	...
	rate		NUMBER			CHECK (rate BETWEEN 0 AND 5),
	ratecount 	NUMBER			DEFAULT 0,
	wdate		DATE			DEFAULT SYSDATE
);

ALTER TABLE JYBOOK DROP CONSTRAINT SYS_C007587 CASCADE;
SELECT * FROM ALL_CONSTRAINTS
WHERE TABLE_NAME = 'JYBOOK';

ALTER TABLE JYBOOK ADD (TITLESEQ NUMBER);

ALTER TABLE JYBOOK ADD CONSTRAINT JYBOOK_TITLE_FK 
FOREIGN KEY(TITLESEQ) REFERENCES JYBOOKTITLE(SEQ);

ALTER TABLE JYBOOK ADD CONSTRAINT JYBOOK_USER_FK 
FOREIGN KEY(id)	REFERENCES JYUSER(id);

CREATE SEQUENCE SEQ_JYBOOK 
START WITH 1 INCREMENT BY 1;

SELECT NVL(count(*), 0) AS cnt FROM JYBOOK;

SELECT seq, id, title, content, readcount, rate, ratecount, wdate 
FROM (
SELECT ROW_NUMBER() over(ORDER BY seq DESC) AS rnum,	
seq, id, title, content, readcount, rate, ratecount, wdate 
FROM JYBOOK
) a WHERE rnum BETWEEN 1 AND 30;

CREATE TABLE JYBOOKRATECOUNT(
	SEQ		NUMBER			NOT NULL,
	ID		VARCHAR2(50)	NOT NULL
)

ALTER TABLE JYBOOKRATECOUNT ADD CONSTRAINT JYBOOKRATE_SEQ_FK
FOREIGN KEY(SEQ) REFERENCES JYBOOK(SEQ);

ALTER TABLE JYBOOKRATECOUNT ADD CONSTRAINT JYBOOKRATE_ID_FK 
FOREIGN KEY(ID) REFERENCES JYUSER(ID);

CREATE TABLE JYBOOKIMG(
	filename		VARCHAR2(100)		NOT NULL,
	SEQ				NUMBER				NOT NULL
);

ALTER TABLE JYBOOKIMG ADD CONSTRAINT JYBOOK_IMG_FK 
FOREIGN KEY(SEQ) REFERENCES JYBOOK(SEQ);

CREATE TABLE JYBOOKTITLE(
	SEQ		NUMBER			PRIMARY KEY,
...
CREATE SEQUENCE JYBOOKTITLE_SEQ 
START WITH 1 INCREMENT BY 1;
